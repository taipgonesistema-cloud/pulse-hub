...
- `whatsapp-web.js` para o MVP via QR.
...
- estrutura pronta para `whatsapp-web.js` no backend;
...
Implementar integracao com `whatsapp-web.js`:
...
- `puppeteer` declarado explicitamente nas dependencias do backend;
- servico NestJS preparado;
- endpoints operacionais criados;
- fluxo real depende da sessao ser iniciada no ambiente com Chromium compativel;
- Dockerfile pronto para EasyPanel com `chromium` instalado e caminho configurado.
...
1. integrar `whatsapp-web.js` no `apps/server`;
...
- conexao via WhatsApp Web sem API oficial pode sofrer instabilidade e mudancas externas;
